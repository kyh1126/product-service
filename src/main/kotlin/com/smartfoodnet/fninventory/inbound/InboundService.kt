...
class InboundService(
    ...
    val inboundExpectedDetailService: InboundExpectedDetailService,
    ...
    private fun getBasicProduct(basicProductCode: String): BasicProduct {
        return basicProductRepository.findByCode(basicProductCode)
            ?: throw NoSuchElementException("기본상품(${basicProductCode})을 찾을 수 없습니다.")
    }
    ...
    fun getInboundActualDetail(partnerId: Long, expectedId: Long) : GetInboundDetailModel {
        val inboundExpectedDetail =
            inboundExpectedDetailService.getInboundExpectedDetail(expectedId)
        return GetInboundDetailModel.toDto(inboundExpectedDetail)
    }
    ...
    fun cancelInbound(inboundId: Long) {
        ...
        val receivingPlanId = inbound.registrationId
            ?: throw BaseRuntimeException(errorMessage = "입고예정등록번호를 찾을 수 없습니다")
